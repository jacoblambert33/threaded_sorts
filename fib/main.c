...
void *fib_t(void *in) {
	...
	//pthread_join(t, ((void*)left)); //this is dumb but keep it bc i thought it and realize why dumb. 
...
/* i can't let my threads die. they are doing the work. so i can't return from these threads, i.e., i can't use join. 
so - try to use th_e serial fib function inside the threading function. 
	but then i have no way to return the value to the calling function. 
for academic purposes, can i extend the queue to hold the return value and the thread itself. this is all bad. 
	what about a cutoff? let's try a cutoff. 
*/
