...
int THIS_CUTOFF = 8; //8191; //4095; //2047; //1023;//511; //255; //127; //63; //31;//15;
...
void pms(unsigned long long a[], int lo, int hi) {
		...
		struct pms_params p = { .a = a, .result = aux, .tmp = aux, .lo = lo, .hi = hi, .start = lo };
...
// public API
void book_p_merge_sort_entry(unsigned long long A[], int p, int r, unsigned long long B[], int s) {

		struct pms_params params = { .a = A, .result = B, .tmp = B, .lo = p, .hi = r, .start = s };
...
*/
void book_p_merge_sort(unsigned long long A[], int p, int r, unsigned long long B[], int s) {
...
void *book_serial(void *prm) {
	...
	if (n == 1)
		...
		struct pms_params left = { .a = input.a, .result = T, .tmp = T, .lo = input.lo, .hi = q, .start = 1 };
		book_serial((void*)&left);
		//book_p_merge_sort(left.a, left.lo, q, T, 1);
		struct pms_params right = { .a = input.a, .result = T, .tmp = T, .lo = q+1, .hi = r, .start = qp+1 };
...
/* TODO: WARNING - this is broken. valgrind makes me think the issue is with my p_merge.c and binary_search.c files. i need to go back through my tests from the bottom up to correct this. it works on arrays <= 40 but not 50 - where i get a corrupt or double free. however, i need to move on for now so i will use the serial version at this time. 
fwiw, book_serial is buggy too - but the issue happens at 100k items. 10k items appears fine. 
*/
void *book_parallel(void *prm) {
	...
	if (n == 1)
		...
		struct pms_params left = { .a = input.a, .result = T, .tmp = T, .lo = input.lo, .hi = q, .start = 1 };
		struct pms_params right = { .a = left.a, .result = T, .tmp = T, .lo = q+1, .hi = r, .start = qp+1 };
...
void *serial(void *fpms_params) {
		...
		if (len == 1)
			...
			book_api_par_merge(tmp, 1, mid_prime, mid_prime+1, len, result, incoming_s); 
			//api_par_merge_output(left.aux, b, len, 1, mid_prime, mid_prime+1, len, left.start); 
			//api_par_merge_output(left.aux, b, len, 1, mid_prime, mid_prime+1, len, left.start); 
			//free(aux); 
...
void *pms_aux(void *fpms_params) {
		...
		if (len == 1)
			...
			book_api_par_merge(left.tmp, 1, mid_prime, mid_prime+1, len, left.result, left.start); 
