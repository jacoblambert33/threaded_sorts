...
Time taken 4 seconds 4135 milliseconds 404 microseconds
        Cf. Qsort is around 1 millisecond, Insertion sort around 250
milliseconds, and basic merge sort is about 3 milliseconds on my test input.

        - there are way too many threads and the cost to start them is
prohibitive. current threadcount 9995

        - perhaps i'm doing rework?
        - perhaps my implementation is wrong - BUT i see that the array is
sorted in the end.
...
// void pmergesort_aux(unsigned long long a[], unsigned long long aux[], int lo,
// int hi) {
void *pmergesort_aux(void *p_quad_params) {
