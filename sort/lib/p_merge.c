...
// public API 
void api_par_merge(unsigned long long haystack[], int h_len, int p1, int r1, int p2, int r2, int p3) {
		int total_len = (r1- p1) + 1 + (r2 - p2)+1; 
		//unsigned long long *result = malloc(total_len * sizeof(*result)); 
		unsigned long long *result = malloc(h_len * sizeof(unsigned long long)); 
		if (!result) {
...
void api_par_merge_output(unsigned long long haystack[], unsigned long long output[], int h_len, int p1, int r1, int p2, int r2, int p3) {
		//int total_len = (r1- p1) + 1 + (r2 - p2)+1; 

		struct pmerge_params p = { .input = haystack, .result = output, .beg_left = p1, .end_left = r1, .beg_right = p2, .end_right = r2, .beg_reslt = p3};
...
void *par_merge(void *params) {
	...
	// new feature; if short array use the serial version rather than create a new thread. 
	if ((len_left + len_right) <= CUTOFF) {

		//printf("len each: %d, %d\n", len_left, len_right);
		parallel_merge(left.input, left.result, left.beg_left, left.end_left, left.beg_right, left.end_right, left.beg_reslt); 
		return NULL;
	}
	...
	pthread_t t; 
	pthread_create(&t, NULL, par_merge, &right); 	
...
// ALT public API 
void book_api_par_merge(unsigned long long haystack[], int p1, int r1, int p2, int r2, unsigned long long result[], int p3) {
		//int total_len = (r1- p1) + 1 + (r2 - p2)+1; 
